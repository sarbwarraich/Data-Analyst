...
type MainView = 'curriculum' | 'lab' | 'cheatsheet' | 'flashcards';

export default function App() {
  const [currentView, setCurrentView] = useState<MainView>('curriculum');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-bold tracking-tight text-white">Data Analyst Hub</span>
              ...
              <p className="text-[10px] text-slate-400 font-mono hidden sm:block">
                Visual Curriculum • Interactive Simulators • Gemini AI
              </p>
            </div>
          </div>
          ...
          <div className="relative flex-1 max-w-xs hidden md:block">
            ...
            {searchResults.length > 0 && (
              <div className="absolute left-0 right-0 top-10 bg-slate-900 border border-slate-800 rounded-lg shadow-2xl max-h-80 overflow-y-auto z-50 p-2 space-y-1">
                {searchResults.map(({ subject, topic }) => (
                  <button
                    key={topic.id}
                    onClick={() => handleSelectTopic(subject.id, topic.id)}
                    className="w-full text-left p-2 rounded hover:bg-slate-800 transition flex items-center justify-between text-xs cursor-pointer"
                  >
                    <div>
                      <div className="font-semibold text-white">{topic.title}</div>
                      <div className="text-[10px] text-slate-400">{subject.title}</div>
                    </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={toggleTheme}
              className="p-2 sm:px-3 sm:py-1.5 rounded-lg border border-slate-700/80 bg-slate-800/90 hover:bg-slate-700 text-slate-200 flex items-center gap-2 text-xs font-semibold transition shadow-sm cursor-pointer"
              title={
                theme === 'dark'
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg bg-slate-800 text-slate-300 cursor-pointer"
          ...
          <div className="lg:hidden bg-slate-900 border-b border-slate-800 px-4 py-3 space-y-2">
            <button
              ...
              className={`w-full text-left p-2 rounded text-xs flex items-center gap-2 ${
                currentView === 'curriculum' ? 'bg-indigo-600 text-white font-bold' : 'text-slate-300'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left p-2 rounded text-xs flex items-center gap-2 ${
                currentView === 'lab' ? 'bg-indigo-600 text-white font-bold' : 'text-slate-300'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left p-2 rounded text-xs flex items-center gap-2 ${
                currentView === 'cheatsheet' ? 'bg-indigo-600 text-white font-bold' : 'text-slate-300'
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left p-2 rounded text-xs flex items-center gap-2 ${
                currentView === 'flashcards' ? 'bg-indigo-600 text-white font-bold' : 'text-slate-300'
              }`}
            >
            ...
            <button
              ...
              className="w-full text-left p-2.5 rounded text-xs flex items-center justify-between bg-slate-800/80 text-slate-200 border border-slate-700/80 cursor-pointer"
            >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {currentView === 'curriculum' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <aside className="lg:col-span-4 bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-4 shadow-xl">
              {/* Header Title */}
              <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                ...
                <span className="text-[10px] font-mono text-slate-400">5 M.Tech Subjects</span>
              </div>
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-3.5 space-y-2.5 shadow-sm">
                ...
                <div className="w-full h-2.5 bg-slate-800/80 rounded-full overflow-hidden border border-slate-700/40 relative">
                  <div
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400">
                  <span>
                    <strong className="text-slate-200">{completedCount}</strong> of {totalTopics}{' '}
                    Topics Completed
                  </span>
                  {completedCount > 0 && (
                    <button
              ...
              <div className="space-y-3">
                {CURRICULUM.map((subject) => {
                  ...
                  return (
                    <div
                      key={subject.id}
                      className="rounded-lg border border-slate-800 overflow-hidden bg-slate-950/60"
                    >
                      {/* Subject Button */}
                      <button
                        onClick={() => {
                        ...
                        <div className="p-2 space-y-1 bg-slate-950/90 border-t border-slate-800/80">
                          {subject.topics.map((topic) => {
                            ...
                            return (
                              <div
                                key={topic.id}
                                className={`w-full rounded text-xs transition flex items-center justify-between group ${
                                  isTopicSelected
                                    ? 'bg-indigo-600 text-white font-semibold shadow-sm'
                              ...
                              >
                                ...
                                >
                                  ...
                                  <span
                                    className={`truncate ${
                                      isCompleted && !isTopicSelected ? 'text-slate-300' : ''
                                    }`}
                                  >
                                    {topic.title}
                                  </span>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 text-slate-100 shadow-xl flex flex-wrap items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400">8 Mathematical & Algorithmic Simulators</span>
                </div>
                <h2 className="text-2xl font-bold text-white mt-1">Interactive Simulation Lab</h2>
      ...
      <footer className="border-t border-slate-800/80 bg-slate-950 py-6 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          <span className="font-mono text-[11px] text-slate-400">
            Powered by Google Gemini AI & Interactive Visualizations
          </span>
        </div>
      </footer>
    </div>
